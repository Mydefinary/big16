...
#!/usr/bin/env python3
"""
Build a FAISS vector index from a CSV of webtoon storylines.

Usage:
  python build_webtoon_faiss.py \
...
Requirements:
  pip install -U pandas faiss-cpu langchain langchain-community langchain-openai langchain-text-splitters python-dotenv
  export OPENAI_API_KEY=your_key

Notes:
  - Uses OpenAI text-embedding-3-large by default. Change via --embedding-model.
  - Persists FAISS index to --db directory via FAISS.save_local (default: ./db1).
"""
...
def build_faiss(
    ...
    # Ensure output dir exists (FAISS will create files under this directory)
    ...
    # Create FAISS index via LangChain and save locally
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a FAISS index from webtoon storyline CSV")
    parser.add_argument("--csv", type=str, default="./webtoon_storyline_data.csv", help="Path to the CSV file")
    parser.add_argument("--db", type=str, default="./db1", help="Directory to persist FAISS index")
    ...
    parser.add_argument("--recreate", action="store_true", help="Delete existing index directory before building")
...
def main():
    ...
    if args.recreate and db_dir.exists():
        print(f"[info] Removing existing index directory: {db_dir}")
    ...
    print("\n[success] FAISS index build complete!")
    print(f"  - Index path     : {db_dir}")
