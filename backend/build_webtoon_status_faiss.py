#!/usr/bin/env python3
"""
Build a FAISS vector DB for the '현황 제공' feature from a CSV of webtoon KPIs / status.

This script is a FAISS-backed port of the original Chroma builder.
It will:
- Read the CSV
- Create a natural-language summary per row (if a dedicated text column isn't provided)
- Chunk -> embed (OpenAI) -> persist to FAISS (./db2 by default)

Quickstart:
  pip install -U pandas langchain langchain-community langchain-openai langchain-text-splitters
  # If FAISS wheels are not present, install one of:
  #   pip install faiss-cpu           # most Linux & Windows (CPU)
  #   pip install faiss-gpu           # GPU-enabled environments
  # macOS (arm64) often works with faiss-cpu as well

  export OPENAI_API_KEY=YOUR_KEY
  python build_webtoon_status_faiss.py \
    --csv /mnt/data/webtoon_data_new.csv \
    --db ./db2 \
    --index-name webtoon_status \
    --chunk-size 700 \
    --chunk-overlap 120 \
    --recreate
...
- To load the index later (example shown at bottom), use FAISS.load_local
"""
...
def build_faiss(
    ...
    *,
    ...
    # Persist to disk. This writes two files under persist_directory / index_name
    #   - index.faiss (the ANN index)
    #   - index.pkl   (the docstore + metadata)
    vectorstore.save_local(folder_path=str(persist_directory), index_name=index_name)


# ---------- CLI ----------
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build FAISS DB for webtoon status CSV")
    p.add_argument("--csv", type=str, default="./webtoon_data_new.csv", help="Path to the CSV file")
    p.add_argument("--db", type=str, default="./db2", help="Directory to persist FAISS DB (default: ./db2)")
    p.add_argument("--index-name", type=str, default="webtoon_status", help="FAISS index name (used as file prefix)")
...
def main():
    ...
    print("\n[how-to] Loading this FAISS index later in Python:")
    print("""
from langchain_community.vectorstores import FAISS
from langchain_openai import OpenAIEmbeddings
...
vs = FAISS.load_local(
    folder_path="./db2",
    index_name="webtoon_status",
    embeddings=emb,
    allow_dangerous_deserialization=True,
)

query = "어제 대비 조회수 급상승 작품"
print(vs.similarity_search(query, k=5))
""")
